...
class UlEncoerModel(torch.nn.Module):
    ...
    def forward(self, observation):
        if not isinstance(observation, torch.Tensor):
            # observation = torch_utils.tensor(observation.view(observation.size(0), -1), self.device)
...
class ContrastModel(torch.nn.Module):
    ...
    def __init__(self, n_actions, latent_size, anchor_hidden_sizes, device='cpu'):
        super().__init__()
